...
class _VolunteerDashboardScreenState extends State<VolunteerDashboardScreen> {
  ...
  Future<void> _loadAll() async {
    await Future.wait([_loadProfile(), _loadOrders(), _loadTasks(), _loadStats()]);
  }
  ...
  Future<void> _loadOrders() async {
    try {
      ...
      if (token != null) {
        ...
        if (mounted) {
          setState(() {
            _availableOrders = all.where((t) => t['status'] == 'AVAILABLE').toList();
  ...
  Future<void> _claimTask(int donationId) async {
    try {
      ...
      if (msg.contains("error1")) {
        ...
        _showErrorDialog("Approval Required", "You are not approved yet. Contact admin.");
      } else {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error: ${msg.replaceAll("Exception:", "")}'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  void _showErrorDialog(String title, String msg) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(title, style: const TextStyle(color: Colors.red, fontWeight: FontWeight.w800)),
        content: Text(msg),
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("Accept this order?", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
  ...
  Future<void> _distributeWithoutPhoto(String pickupToken) async {
    try {
      ...
      final token = prefs.getString('token');
      if (token == null) return;
      final url = Uri.parse('${_apiService.baseUrl}/donations/distribute/withoutphoto?token=$pickupToken');
      final response = await http.post(url, headers: {'Authorization': 'Bearer $token'});
      if (response.statusCode == 200) {
        if (!mounted) return;
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Distribution confirmed! ✅'), backgroundColor: Colors.green));
        _loadAll();
      } else {
  ...
  void _showTaskDetails(Map<String, dynamic> task) {
    ...
    String dateLabel = '';
    try {
      dateLabel = DateFormat('MMM dd, hh:mm a').format(DateTime.parse(task['mustPickupBy']));
    } catch (_) {}
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  ...
                  style: TextStyle(color: _statusColor(task['status']), fontWeight: FontWeight.w800, fontSize: 13),
                ),
              ),
              const SizedBox(height: 20),
              ...
              if (token.isNotEmpty) ...[
                Container(
                  ...
                  child: QrImageView(data: token, version: QrVersions.auto, size: 160.0),
                ),
                const SizedBox(height: 8),
                Text("Show this QR to restaurant", style: TextStyle(color: AppColors.textSecondary.withOpacity(0.7), fontSize: 12, fontWeight: FontWeight.w600)),
                const SizedBox(height: 20),
              ],

              // Info rows
              _DetailRow(icon: Icons.restaurant_rounded, label: "Restaurant", value: task['restaurantName'] ?? 'N/A'),
              _DetailRow(icon: Icons.fastfood_rounded, label: "Item", value: task['foodDescription'] ?? 'N/A'),
              _DetailRow(icon: Icons.access_time_rounded, label: "Pickup By", value: dateLabel),
              _DetailRow(icon: Icons.info_outline_rounded, label: "Status", value: _statusLabel(task['status'])),
              const SizedBox(height: 20),
              ...
              if (task['status'] == 'PICKED_UP') ...[
                ...
                const Align(
                  ...
                  child: Text("Confirm Distribution", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
                ),
                const SizedBox(height: 12),
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text("Distribute (No Photo)", style: TextStyle(fontWeight: FontWeight.w700)),
                  ),
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ListTile(
                                ...
                                onTap: () { Navigator.pop(ctx); _distributeWithPhoto(token, ImageSource.camera); },
                              ),
                              ListTile(
                                leading: Container(
                                  padding: const EdgeInsets.all(8),
                                ...
                                onTap: () { Navigator.pop(ctx); _distributeWithPhoto(token, ImageSource.gallery); },
                              ),
                              const SizedBox(height: 16),
                            ],
                          ),
                    ...
                    label: const Text("Distribute with Photo", style: TextStyle(fontWeight: FontWeight.w700)),
                  ),
                ),
              ],
              const SizedBox(height: 24),
  ...
  Future<void> _changePassword(String oldPassword, String newPassword) async {
    ...
    try {
      final url = Uri.parse('${_apiService.baseUrl}/volunteer/update/password/$userId?oldPassword=$oldPassword&newPassword=$newPassword');
      final response = await http.post(url, headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json'});
      if (response.statusCode == 200 && response.body.trim() == 'true') {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Failed. Check old password."), backgroundColor: Colors.red));
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error: $e"), backgroundColor: Colors.red));
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: loading
                        ...
                        : const Text("Confirm", style: TextStyle(fontWeight: FontWeight.w800)),
                  ),
                ),
              ],
            ),
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: const Icon(Icons.volunteer_activism_rounded, size: 40, color: Colors.white),
              ),
              const SizedBox(height: 16),
              Text(_organizationName, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
                decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                child: const Text("Verified Partner", style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AppColors.primary)),
              ),
              const SizedBox(height: 24),
              _DetailRow(icon: Icons.person_rounded, label: "President", value: d['presidentFullName']?.toString() ?? 'N/A'),
              _DetailRow(icon: Icons.email_rounded, label: "Email", value: d['email']?.toString() ?? 'N/A'),
              _DetailRow(icon: Icons.phone_rounded, label: "Phone", value: d['phoneNumber']?.toString() ?? 'N/A'),
              const SizedBox(height: 20),
              Row(
                children: [
                  Expanded(
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () { Navigator.pop(ctx); _showChangePasswordDialog(); },
  ...
  void _showEditDialog() {
    final orgCtrl = TextEditingController(text: _volunteerDetails?['organizationName']);
    final presCtrl = TextEditingController(text: _volunteerDetails?['presidentFullName']);
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                _editField(orgCtrl, "Organization Name", Icons.business_rounded),
                const SizedBox(height: 14),
                _editField(presCtrl, "President Name", Icons.person_rounded),
                const SizedBox(height: 14),
                _editField(phoneCtrl, "Phone", Icons.phone_rounded, keyboardType: TextInputType.phone),
                const SizedBox(height: 24),
                Row(
                  children: [
                    Expanded(child: TextButton(onPressed: loading ? null : () => Navigator.pop(ctx), child: const Text("Cancel", style: TextStyle(fontWeight: FontWeight.w700)))),
                    ...
                    Expanded(
                      ...
                      child: ElevatedButton(
                        onPressed: loading ? null : () async {
                          setD(() => loading = true);
                          try {
                            await _updateProfile(orgCtrl.text, presCtrl.text, phoneCtrl.text);
  ...
  Widget _editField(TextEditingController ctrl, String label, IconData icon, {TextInputType? keyboardType}) {
    return TextField(
      controller: ctrl,
  ...
  Color _statusColor(String? s) {
    switch (s) {
      ...
      case 'CLAIMED': return AppColors.secondary;
      case 'PICKED_UP': return Colors.deepOrange;
  ...
  Widget _buildNavItem(int index, IconData icon, String label, {int badge = 0}) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        padding: EdgeInsets.symmetric(horizontal: selected ? 20 : 16, vertical: 10),
        decoration: BoxDecoration(
      ...
      children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _availableOrders.isEmpty ? "No orders right now" : "${_availableOrders.length} order${_availableOrders.length == 1 ? '' : 's'} waiting",
                      style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textSecondary.withOpacity(0.7)),
                    ),
                  ],
                ),
              ),
        ...
        SizedBox(
          ...
          child: ListView(
            ...
            children: [
              _MiniStat(icon: Icons.verified_user_rounded, label: "Credit", value: "$_creditScore", color: const Color(0xFF6366F1)),
              _MiniStat(icon: Icons.local_shipping_rounded, label: "Active", value: "$_activeCount", color: AppColors.secondary),
              _MiniStat(icon: Icons.check_circle_rounded, label: "Done", value: "$_finishedCount", color: AppColors.primary),
              _MiniStat(icon: Icons.calendar_month_rounded, label: "Month", value: "$_monthCount", color: AppColors.info),
            ],
          ),
        ),
        const SizedBox(height: 12),
        ...
        Expanded(
          child: _isLoadingOrders
              ...
              : _availableOrders.isEmpty
                  ? _buildEmptyState("No orders available", "Pull to refresh — new orders appear instantly!", Icons.delivery_dining_rounded)
                  : RefreshIndicator(
                      onRefresh: _loadAll,
                      color: AppColors.primary,
  ...
  Widget _buildMyDeliveriesPage() {
    ...
    final completed = _myTasks.where((t) => t['status'] == 'COMPLETED').toList();
                      ...
                      child: ListView(
                        ...
                        children: [
                          if (active.isNotEmpty) ...[
                            _sectionHeader("Active", active.length, Colors.deepOrange),
                            ...active.map((t) => _DeliveryCard(task: t, onTap: () => _showTaskDetails(t), statusColor: _statusColor(t['status']), statusLabel: _statusLabel(t['status']))),
                          ],
                          if (completed.isNotEmpty) ...[
                            const SizedBox(height: 16),
                            _sectionHeader("Completed", completed.length, AppColors.primary),
                            ...completed.map((t) => _DeliveryCard(task: t, onTap: () => _showTaskDetails(t), statusColor: _statusColor(t['status']), statusLabel: _statusLabel(t['status']))),
                          ],
                        ],
                      ),
                    ),
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("Your Impact", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: -0.5)),
            const SizedBox(height: 20),
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("Credit Score", style: TextStyle(color: Colors.white.withOpacity(0.85), fontSize: 14, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 4),
                        Text(
                          "$_creditScore",
                        ...
                        Container(
                          ...
                          child: Text(
                            _creditScore >= 80 ? "⭐ Excellent" : _creditScore >= 50 ? "👍 Good" : "📈 Building",
            ...
            Row(
              children: [
                Expanded(child: _StatTile(label: "Total Missions", value: "$_totalCount", icon: Icons.assignment_turned_in_rounded, color: AppColors.info)),
                const SizedBox(width: 12),
                Expanded(child: _StatTile(label: "This Month", value: "$_monthCount", icon: Icons.calendar_today_rounded, color: AppColors.secondary)),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: _StatTile(label: "Active Now", value: "$_activeCount", icon: Icons.pending_actions_rounded, color: Colors.deepOrange)),
                const SizedBox(width: 12),
                Expanded(child: _StatTile(label: "Completed", value: "$_finishedCount", icon: Icons.check_circle_rounded, color: AppColors.primary)),
              ],
            ),
            const SizedBox(height: 24),
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textPrimary.withOpacity(0.8)),
                    ),
                  ),
                ],
              ),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(icon, size: 52, color: AppColors.primary.withOpacity(0.4)),
          ),
          const SizedBox(height: 20),
          Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
          const SizedBox(height: 6),
          Text(subtitle, style: TextStyle(fontSize: 13, color: AppColors.textSecondary.withOpacity(0.7), fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
...
class _OrderCard extends StatefulWidget {
  ...
  const _OrderCard({required this.order, required this.onAccept, required this.index});
      ...
      child: SlideTransition(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(20),
            border: isUrgent ? Border.all(color: Colors.redAccent.withOpacity(0.2), width: 1) : null,
            boxShadow: [
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.all(10),
                          decoration: BoxDecoration(
                            color: isUrgent ? Colors.red.withOpacity(0.07) : AppColors.secondary.withOpacity(0.07),
                            borderRadius: BorderRadius.circular(14),
                          ),
                          child: Icon(
                            Icons.store_rounded,
                            color: isUrgent ? Colors.redAccent : AppColors.secondary,
                            size: 22,
                          ),
                        ),
                        const SizedBox(width: 12),
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                const Text("URGENT", style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 0.5)),
                              ],
                            ),
                          ),
                      ],
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        _Chip(icon: Icons.timer_rounded, text: "${hours}h left"),
                        const Spacer(),
                        Container(
                          ...
                          child: const Text("Accept", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 13)),
                        ),
                      ],
                    ),
                  ],
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(text, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textSecondary)),
        ],
      ),
    );
  }
...
class _DeliveryCard extends StatelessWidget {
  ...
  const _DeliveryCard({required this.task, required this.onTap, required this.statusColor, required this.statusLabel});
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(
                    isCompleted ? Icons.check_circle_rounded : Icons.delivery_dining_rounded,
                    color: statusColor,
                    size: 24,
                  ),
                ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: color.withOpacity(0.7))),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 2),
          Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textSecondary.withOpacity(0.6))),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.textSecondary.withOpacity(0.5))),
                const SizedBox(height: 2),
